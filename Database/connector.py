...
class Paternoster(Model):
    ...
    pat_pos = fields.ForeignKeyField(
        "models.PaternosterPositions", related_name="pat_positions"
    )
    pat_part_number = fields.ForeignKeyField(
        "models.PartNumber", related_name="part_number"
    )
...
async def create_box(
    serial_number: str, type_id: int = None, last_cleand=datetime.now(), uses=0
):
...
async def create_Type(name: str, max_num_uses: int, cleaning_period: int):
    ...
    """
    await Types.create(
        name=name, max_num_uses=max_num_uses, cleaning_period=cleaning_period
    )
...
async def alter_box(
    box_type_name: str, box_type_name_new: str, new_period: int, new_uses=0
):
    """
